...
report, the 
the data are cleaned and carefully prepared for investigation, the dataset is then  
...
The two records missing \textit{sex} and all numerical features were removed as imputation is unlikely to be reliable. 
The remaining nine records are missing only the \textit{sex} attribute. Figure~\ref{fig:sex} shows the physical 
attributes of the male and female of each species differ statistically, making it reasonable to 
consider imputing \textit{sex} for those records would be reliable. Following standardization, 
...
significantly influenced by the island inhabited. \textit{island} is thus unlikely to be a confounding factor.
...
The code is available on Github \cite{TimAIRepo} and was written in Python 3.11 \cite{python311} 
...
An accuracy improvement was achieved by creating a separate set of clusters for each \textit{sex}.
...
Figure~\ref{fig:CVA}(b) plots \textit{bill length} against \textit{bill depth} and shows the SVM boundary that best 
separates Adelie and Chinstrap. An improvement in accuracy
was apparent when separate SVM models were developed for each \textit{sex}.
